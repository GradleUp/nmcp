...
If your account was created before, and you used to log in to [https://oss.sonatype.org](https://oss.sonatype.org) or [https://central.sonatype.com/](https://central.sonatype.com/), you now need to login to [https://central.sonatype.com/](https://central.sonatype.com/).

Your previous credentials should work. If not, contact the central support at `central-support@sonatype.com`. See [this page](https://central.sonatype.org/pages/ossrh-eol/) for more details.
...
The Portal API is the recommended way moving forward though and it is recommended to migrate.
...
* The `zip` approach makes deployments makes everything more atomic. No more weird half-closed staging repos!
* More generally, uploading everything all at once simplifies everything and removes the heuristics where staging repositories [were created based on your IP], causing split repos and other issues.
...
The status codes are not documented, but a [community maintained list is available in the source code](https://github.com/gradleup/nmcp/blob/01d3dd57f1a02227d4466a0e2a22d3be8cde3336/nmcp-tasks/src/main/kotlin/nmcp/transport/transport.kt#L133). If you bump into an unknown status code, please send a PR!
